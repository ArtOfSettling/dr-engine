...
mod tests {
    ...
    #[test]
    fn correctly_takes() {
        let input: Vec<&str> = vec!["a", "ab", "abc", "abcd"];
        let expected_result: Vec<&str> = vec!["a", "ab", "abc"];
        let iter: Vec<&str> = input.iter()
            .take_until(|a| (*a).len() == 3)
            .map(|a| *a)
            .collect();
        assert_eq!(iter, expected_result)
    }
}
